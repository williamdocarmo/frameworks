...
public class SOAPClientHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public boolean handleMessage(SOAPMessageContext ctx) {
		...
		try {
			boolean outgoing = (Boolean) ctx.get(SOAPMessageContext.MESSAGE_OUTBOUND_PROPERTY);
			if (outgoing) {
				HashMap<String, List<String>> headers = (HashMap<String, List<String>>) ctx.get(SOAPMessageContext.HTTP_REQUEST_HEADERS);
				List<String> secureToken = new ArrayList<>();
				secureToken.add("Basic "+Base64.getEncoder().encodeToString("santanu:santanupass".getBytes()));
				// headers.put("Authorization", secureToken);
			}
